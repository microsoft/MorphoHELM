"""
KNN Recall@1 + mAP benchmark sweep.

Paradigms: NR, NSB, NSS, NSL (Not Same Layout), NSSL (Not Same Source Layout)

Usage:
    python run_knn_sweep.py \
...
parser.add_argument("--features-base", required=True, help="Base dir with {dataset}/ subdirs of normalized parquets")
parser.add_argument("--results-dir", required=True, help="Output dir for results pkl")
parser.add_argument(
    "--only-dataset",
...
def main():
    ...
    if args.only_dataset in ("both", "cpg-tgt2"):
        ...
        for model, fname in MODELS.items():
            ...
            knn = run_knn(Xn, compounds, batches, sources, wells, tgt2_paradigms)
            tgt2_knn[model] = knn

            mp = run_map(Xn, compounds, batches, sources, wells, tgt2_paradigms)
            tgt2_map[model] = mp

            elapsed = time.time() - t0
            for p in tgt2_paradigms:
                print(f"    {p}: KNN={knn[p]['recall']:.1f}% ({knn[p]['n_eligible']}q)  "
                      f"mAP={mp[p]['mAP']:.2f}% ({mp[p]['n_queries']}q)")
    ...
    for model, fname in MODELS.items():
        ...
        knn = run_knn(Xn, compounds, batches, sources, wells, cpg_paradigms)
        cpg_knn[model] = knn

        mp = run_map(Xn, compounds, batches, sources, wells, cpg_paradigms)
        cpg_map[model] = mp

        elapsed = time.time() - t0
        for p in cpg_paradigms:
            print(f"    {p}: KNN={knn[p]['recall']:.2f}% ({knn[p]['n_eligible']}q)  "
                  f"mAP={mp[p]['mAP']:.4f}% ({mp[p]['n_queries']}q)")
        print(f"    [{elapsed:.0f}s]")
    ...
    if not tgt2_map or not cpg_map:
        out = {
    ...
    # Sort models by No Restriction mAP on tgt2 (use consistent order)
    ...
    for metric, get_val, cbar_label, fname_suffix in [
        ("mAP", lambda res, m, p: res[m][p]['mAP'], 'mAP (%)', 'map'),
        ("KNN Recall@1", lambda res, m, p: res[m][p]['recall'], 'Recall@1 (%)', 'knn'),
    ]:
